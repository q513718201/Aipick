...
public class AppCombinedChartRenderer extends DataRenderer {
    ...
    /**
     * all reders for the different kinds of data this combined-renderer can draw
     */
    protected List<DataRenderer> mRenders = new ArrayList<DataRenderer>(5);
    ...
    /**
     * Creates the renders needed for this combined-renderer in the required order. Also takes the DrawOrder into
     * consideration.
     */
    public void createRenders() {
            ...
            switch (order) {
                case BAR:
                    if (chart.getBarData() != null)
                        mRenders.add(new BarChartRenderer(chart, mAnimator, mViewPortHandler));
                    break;
                case BUBBLE:
                    if (chart.getBubbleData() != null)
                        mRenders.add(new BubbleChartRenderer(chart, mAnimator, mViewPortHandler));
                    break;
                case LINE:
                    if (chart.getLineData() != null)
                        mRenders.add(new AppLineChartRenderer(chart, mAnimator, mViewPortHandler));
                    break;
                case CANDLE:
                    if (chart.getCandleData() != null)
                        mRenders.add(new CandleStickChartRenderer(chart, mAnimator, mViewPortHandler));
                    break;
                case SCATTER:
                    if (chart.getScatterData() != null)
                        mRenders.add(new ScatterChartRenderer(chart, mAnimator, mViewPortHandler));
                    break;
            }
        }
    }
    ...
    public void initBuffers() {
        ...
        for (DataRenderer renderer : mRenders)
    ...
    public DataRenderer getSubRenderer(int index) {
        if (index >= mRenders.size() || index < 0)
            return null;
        else
            return mRenders.get(index);
    }
